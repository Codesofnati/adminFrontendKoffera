...
import { WrenchScrewdriverIcon, PlusIcon, PencilIcon, TrashIcon, CheckCircleIcon, ExclamationTriangleIcon } from "@heroicons/react/24/outline"; // Install @heroicons/react if needed
...
export default function ServiceManager() {
  ...
  const addService = async () => {
    ...
    try {
      const res = await fetch("https://newsss.koffera.workers.dev/services", {
        method: "POST",
        body: formData,
      });
          ...
          <div className="space-y-4">
            {services.length === 0 ? (
              ...
              services.map((s) => (
                <div
                  key={s.id}
                  className="border border-gray-200 p-4 rounded-lg hover:bg-gray-50 transition-all duration-300 group"
                >
                  {editingServiceId === s.id ? (
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                      ...
                      <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <button
                          className="bg-gradient-to-r from-yellow-500 to-orange-600 hover:from-yellow-600 hover:to-orange-700 text-white px-4 py-2 rounded-lg font-medium transition-all duration-300 flex items-center gap-2"
                          onClick={() => {
                        ...
                        <button
                          className="bg-gradient-to-r from-red-500 to-pink-600 hover:from-red-600 hover:to-pink-700 text-white px-4 py-2 rounded-lg font-medium transition-all duration-300 flex items-center gap-2"
